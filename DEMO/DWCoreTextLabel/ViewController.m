...
- (void)viewDidLoad {
    ...
    label.textInsets = UIEdgeInsetsMake(50, 10, 10, 10);
    label.textColor = [UIColor blueColor];
//    label.numberOfLines = 2;
    [self.view addSubview:label];
    label.exclusionPaths = @[[UIBezierPath bezierPathWithRect:CGRectMake(10, 50, 120, 120)]].mutableCopy;
    
    CGFloat width = [UIScreen mainScreen].bounds.size.width;
    CGFloat padding = 10;
    width -= (padding * 2 + label.textInsets.left + label.textInsets.right);
    
    [label dw_InsertImage:[UIImage imageNamed:@"2.jpg"] withImageID:@"URL" size:CGSizeMake(width, width * 9 / 16.0) padding:padding descent:0 atLocation:91 target:self selector:@selector(clickPic)];
    [label dw_DrawImage:[UIImage imageNamed:@"oldDriver"] withImageID:@"URL" path:[UIBezierPath bezierPathWithOvalInRect:CGRectMake(10,50, 120, 120)] margin:0 drawMode:(DWTextImageDrawModeCover) target:self selector:@selector(clickHeader)];
